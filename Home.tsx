...
import { Trophy, BookOpen, GraduationCap, ChevronRight, Award, Zap, Brain, UserPlus, ShieldCheck } from 'lucide-react';
          ...
          >
            <div className="relative z-10 bg-[#1e293b] p-10 rounded-[3.5rem] shadow-2xl border-2 border-white/5">
              ...
              <div className="grid grid-cols-2 gap-5">
                <div className="bg-white/5 p-6 rounded-3xl border border-white/10 flex items-center gap-4 group hover:bg-emerald-600/10 transition-colors">
                  <div className="bg-emerald-600 p-3 rounded-2xl"><BookOpen className="text-white w-6 h-6" /></div>
                  <div><p className="text-[10px] text-slate-500 font-black uppercase">Kurikulum</p><p className="font-black text-white uppercase tracking-tighter">NASIONAL</p></div>
                </div>
                <div className="bg-white/5 p-6 rounded-3xl border border-white/10 flex items-center gap-4 group hover:bg-blue-600/10 transition-colors">
                  <div className="bg-blue-600 p-3 rounded-2xl"><Trophy className="text-white w-6 h-6" /></div>
                  <div><p className="text-[10px] text-slate-500 font-black uppercase">Prestasi</p><p className="font-black text-white uppercase tracking-tighter">JUARA</p></div>
                </div>
              </div>
            </div>
          </MotionDiv>
      ...
      <section className="py-32 bg-white">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {[
              { level: "SD", color: "blue", title: "SEKOLAH DASAR", icon: <BookOpen />, subjects: "HARD" },
              { level: "SMP", color: "emerald", title: "MENENGAH PERTAMA", icon: <GraduationCap />, subjects: "EXTREME" },
              { level: "SMA", color: "purple", title: "MENENGAH ATAS", icon: <Award />, subjects: "IMPOSSIBLE" }
            ].map((item, idx) => (
              <MotionDiv 
                whileHover={{ y: -15 }}
                key={idx}
