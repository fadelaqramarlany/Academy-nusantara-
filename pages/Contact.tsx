    ...
    <div className="min-h-screen bg-slate-50 py-20 px-4">
      <div className="max-w-7xl mx-auto">
        <div className="grid md:grid-cols-2 gap-16">
          ...
          {/* Contact Form */}
          <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-xl border border-slate-200 min-h-[480px] flex flex-col justify-center">
            <AnimatePresence mode="wait">
              {!isSubmitted ? (
                <motion.div key="form" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div>
                      ...
                      <textarea id="message" required rows={3} className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none transition" placeholder="Ketik pesan..." value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})}></textarea>
                    </div>
                    <button type="submit" className="w-full bg-blue-600 text-white py-4 rounded-2xl font-bold text-lg shadow-xl hover:bg-blue-700 active:scale-95 transition-all flex items-center justify-center gap-2">
                ...
                <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-8">
                  ...
                  <h2 className="text-2xl font-black text-slate-900 mb-2 uppercase tracking-tighter">PESAN TERKIRIM!</h2>
                  <p className="text-slate-600 text-sm mb-6">Admin akan segera menghubungi Anda melalui WhatsApp atau email.</p>
                  <button onClick={() => { setIsSubmitted(false); setFormData({ name: '', school: '', message: '' }); }} className="px-6 py-3 bg-slate-900 text-white rounded-xl font-bold active:scale-95 transition-all uppercase tracking-widest text-xs">Kirim Pesan Lagi</button>
                </motion.div>
              )}
            </AnimatePresence>
