          ...
          <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-xl border border-slate-200 min-h-[500px] flex flex-col justify-center">
            {isSubmitted ? (
              <MotionDiv initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-8">
                ...
                <p className="text-slate-500 font-bold mb-8 uppercase tracking-widest text-[10px] leading-relaxed">Admin akan menghubungi Anda segera melalui WhatsApp atau Email.</p>
                <button onClick={() => setIsSubmitted(false)} className="bg-slate-900 text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-blue-600 transition-all active:scale-95 shadow-xl">
                  Kirim Pesan Lagi
                </button>
              </MotionDiv>
            ) : (
              <>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <button type="submit" className="w-full bg-blue-600 text-white py-5 rounded-2xl font-bold text-lg shadow-xl hover:bg-blue-700 transition flex items-center justify-center gap-2 active:scale-95 transition-all">
                    Kirim Pesan <Send size={20} />
                  </button>
                </form>
                <p className="text-center text-slate-400 text-sm mt-8 italic text-xs">
