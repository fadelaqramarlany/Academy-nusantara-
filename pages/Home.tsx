...
import { Trophy, BookOpen, GraduationCap, ChevronRight, Award, Zap, Brain, UserPlus, CheckCircle } from 'lucide-react';
           ...
           <div className="bg-white p-12 rounded-[3rem] shadow-2xl border-t-8 border-blue-600">
              {isSubmitted ? (
                <MotionDiv initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-10">
                   ...
                   <h2 className="text-4xl font-black text-slate-900 uppercase tracking-tighter">PENDAFTARAN BERHASIL!</h2>
                   <p className="text-slate-500 font-bold uppercase tracking-widest text-sm mt-4">Data Anda telah tersimpan secara nasional. Silakan pilih jenjang pendidikan untuk memulai kuis.</p>
                   <button onClick={() => setIsSubmitted(false)} className="mt-8 text-blue-600 font-black uppercase tracking-widest text-xs hover:underline">Daftar Siswa Lain</button>
                </MotionDiv>
              ) : (
                <>
                  ...
                  <form className="grid md:grid-cols-2 gap-6" onSubmit={(e) => { e.preventDefault(); setIsSubmitted(true); }}>
                    <div className="space-y-4">
                        <label htmlFor="nama_lengkap" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 cursor-pointer">Nama Lengkap</label>
                        <input id="nama_lengkap" type="text" placeholder="Nama sesuai rapor..." required className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none font-bold" />
                    </div>
                    <div className="space-y-4">
                    ...
                    <div className="space-y-4">
                        <label htmlFor="asal_sekolah" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 cursor-pointer">Asal Sekolah</label>
                        <input id="asal_sekolah" type="text" placeholder="Nama sekolah lengkap..." required className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none font-bold" />
                    </div>
                    <div className="space-y-4">
                        <label htmlFor="jenjang" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 cursor-pointer">Jenjang</label>
