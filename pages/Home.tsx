           ...
           <div className="bg-white p-12 rounded-[3rem] shadow-2xl border-t-8 border-blue-600 min-h-[400px] flex flex-col justify-center">
              {isRegistered ? (
                <MotionDiv initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
                  <CircleCheck size={48} className="text-emerald-500 mx-auto mb-4" />
                  <h2 className="text-4xl font-black text-slate-900 uppercase mb-4">PENDAFTARAN BERHASIL!</h2>
                  <button onClick={() => setIsRegistered(false)} className="bg-slate-900 text-white px-8 py-3 rounded-xl font-bold uppercase active:scale-95 transition-all">Daftar Lagi</button>
                </MotionDiv>
              ) : (
                <form className="grid md:grid-cols-2 gap-6" onSubmit={(e) => { e.preventDefault(); setIsRegistered(true); }}>
                  <div className="md:col-span-2 text-center mb-4">
                    <h2 className="text-4xl font-black text-slate-900 uppercase">PENDAFTARAN SISWA</h2>
                  </div>
                  {[
                    ["name", "Nama Lengkap", "Nama sesuai rapor..."],
                    ["nisn", "NISN", "Nomor Induk Siswa..."],
                    ["school", "Asal Sekolah", "Nama sekolah lengkap..."]
                  ].map(([id, label, placeholder]) => (
                    <div key={id} className="space-y-2">
                      <label htmlFor={id} className="block text-[10px] font-black text-slate-400 uppercase ml-2">{label}</label>
                      <input id={id} placeholder={placeholder} required className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none font-bold" />
                    </div>
                  ))}
                  <div className="space-y-2">
                    <label htmlFor="level" className="block text-[10px] font-black text-slate-400 uppercase ml-2">Jenjang</label>
                  ...
                  <button className="md:col-span-2 bg-slate-900 text-white py-5 rounded-2xl font-black uppercase hover:bg-blue-600 transition-all active:scale-95 shadow-xl mt-4">Simpan Data Peserta</button>
                </form>
              )}
           </div>
        </div>
