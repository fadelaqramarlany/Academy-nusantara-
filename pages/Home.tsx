...
import { Trophy, BookOpen, GraduationCap, ChevronRight, Award, Zap, Brain, UserPlus, CheckCircle2 } from 'lucide-react';
...
const Home: React.FC = () => {
  ...
  return (
    <div className="overflow-hidden">
              ...
              <AnimatePresence mode="wait">
                {!isSubmitted ? (
                  ...
                  >
                    ...
                    <form className="grid md:grid-cols-2 gap-6" onSubmit={(e) => { e.preventDefault(); setIsSubmitted(true); }}>
                       <div className="space-y-4">
                          <label htmlFor="name" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 cursor-pointer hover:text-blue-600 transition-colors">Nama Lengkap</label>
                          <input id="name" type="text" placeholder="Nama sesuai rapor..." required className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none font-bold transition-all" />
                       </div>
                       <div className="space-y-4">
                          <label htmlFor="nisn" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 cursor-pointer hover:text-blue-600 transition-colors">NISN</label>
                          <input id="nisn" type="text" placeholder="Nomor Induk Siswa..." required className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none font-bold transition-all" />
                       </div>
                       <div className="space-y-4">
                          <label htmlFor="school" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 cursor-pointer hover:text-blue-600 transition-colors">Asal Sekolah</label>
                          <input id="school" type="text" placeholder="Nama sekolah lengkap..." required className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none font-bold transition-all" />
                       </div>
                       <div className="space-y-4">
                          <label htmlFor="level" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 cursor-pointer hover:text-blue-600 transition-colors">Jenjang</label>
                          <select id="level" className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none font-bold cursor-pointer transition-all">
                             <option>SD</option>
                             <option>SMP</option>
                             <option>SMA</option>
                          </select>
                       </div>
                       <button className="md:col-span-2 bg-slate-900 text-white py-5 rounded-2xl font-black uppercase tracking-widest hover:bg-blue-600 active:scale-95 transition-all shadow-xl mt-4">Simpan Data Peserta</button>
                    </form>
                  </MotionDiv>
                ) : (
                  <MotionDiv
                  ...
                  >
                    ...
                    <h2 className="text-4xl font-black text-slate-900 mb-4 uppercase tracking-tighter">BERHASIL TERDAFTAR!</h2>
                    <p className="text-slate-600 font-bold mb-10 max-w-sm mx-auto uppercase text-sm tracking-wide leading-relaxed">
                      Data Anda telah masuk ke Database Prestasi Nasional. Selamat berjuang!
                    </p>
                    <div className="flex flex-col sm:flex-row justify-center gap-4">
                      <a href="#jenjang" className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-blue-700 active:scale-95 transition-all shadow-lg text-sm">Mulai Kuis</a>
                      <button onClick={() => setIsSubmitted(false)} className="bg-slate-100 text-slate-600 px-8 py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-slate-200 active:scale-95 transition-all text-sm">Daftar Lagi</button>
                    </div>
                  </MotionDiv>
                )}
              </AnimatePresence>
      ...
      {/* Levels Section */}
      <section id="jenjang" className="py-32 bg-white">
