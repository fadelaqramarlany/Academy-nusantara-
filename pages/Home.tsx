...
import { Trophy, BookOpen, GraduationCap, Award, Zap, Brain, UserPlus, CheckCircle } from 'lucide-react';
    ...
    <div className="overflow-hidden">
      ...
      <section id="pendaftaran" className="py-24 bg-slate-50 border-b border-slate-200">
        <div className="max-w-4xl mx-auto px-6">
           <div className="bg-white p-12 rounded-[3rem] shadow-2xl border-t-8 border-blue-600 min-h-[460px] flex flex-col justify-center">
              <AnimatePresence mode="wait">
                {!isRegistered ? (
                  <MotionDiv key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    ...
                    <form className="grid md:grid-cols-2 gap-6" onSubmit={handleRegister}>
                       <div className="space-y-4">
                          ...
                          <input id="name" type="text" placeholder="Nama sesuai rapor..." required className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none font-bold transition-all focus:ring-2 focus:ring-blue-500/20" value={name} onChange={(e) => setName(e.target.value)} />
                       </div>
                       <div className="space-y-4">
                          <label htmlFor="nisn" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">NISN</label>
                          <input id="nisn" type="text" placeholder="Nomor Induk Siswa..." required className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none font-bold transition-all focus:ring-2 focus:ring-blue-500/20" />
                       </div>
                       <div className="space-y-4">
                          ...
                          <input id="school" type="text" placeholder="Nama sekolah lengkap..." required className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none font-bold transition-all focus:ring-2 focus:ring-blue-500/20" />
                       </div>
                       <div className="space-y-4">
                          <label htmlFor="level" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Jenjang</label>
                          <select id="level" className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none font-bold transition-all focus:ring-2 focus:ring-blue-500/20">
                  ...
                  <MotionDiv key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center" role="status" aria-live="polite">
                    <CheckCircle className="w-20 h-20 text-emerald-500 mx-auto mb-6" />
                    <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tighter mb-4">Pendaftaran Berhasil!</h2>
                    <p className="text-slate-500 font-bold uppercase tracking-widest text-sm mb-8">Selamat, {name}! Data Anda telah tersimpan di sistem Academy Nusantara.</p>
                    <button onClick={() => setIsRegistered(false)} className="bg-slate-900 text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-blue-600 transition-all active:scale-95 text-xs">Daftar Lagi</button>
                  </MotionDiv>
                )}
              </AnimatePresence>
           </div>
