      ...
      <section id="pendaftaran" className="py-24 bg-slate-50 border-b border-slate-200">
        <div className="max-w-4xl mx-auto px-6">
           <div className="bg-white p-12 rounded-[3rem] shadow-2xl border-t-8 border-blue-600">
              ...
              {isSubmitted ? (
                <MotionDiv initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="py-12 text-center">
                  <div className="w-24 h-24 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg border-4 border-white">
                    <CircleCheck size={48} aria-hidden="true" />
                  </div>
                  <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tight mb-4">PENDAFTARAN BERHASIL!</h3>
                  <p className="text-slate-600 font-bold mb-8 uppercase tracking-wide">Data Anda telah masuk ke pusat prestasi nasional.</p>
                  <button onClick={() => setIsSubmitted(false)} className="px-10 py-4 bg-slate-900 text-white rounded-2xl font-black uppercase tracking-widest hover:bg-blue-600 transition-all active:scale-95 shadow-xl">Daftar Lagi</button>
                </MotionDiv>
              ) : (
                <form className="grid md:grid-cols-2 gap-6" onSubmit={(e) => { e.preventDefault(); setIsSubmitted(true); }}>
                  {[
                    { label: 'Nama Lengkap', id: 'name', placeholder: 'Nama sesuai rapor...' },
                    { label: 'NISN', id: 'nisn', placeholder: 'Nomor Induk Siswa...' },
                    { label: 'Asal Sekolah', id: 'school', placeholder: 'Nama sekolah lengkap...' }
                  ].map(f => (
                    <div key={f.id} className="space-y-4">
                      <label htmlFor={f.id} className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">{f.label}</label>
                      <input id={f.id} type="text" placeholder={f.placeholder} required className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none font-bold" />
                    </div>
                  ))}
