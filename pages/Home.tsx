           ...
           <div className="bg-white p-12 rounded-[3rem] shadow-2xl border-t-8 border-blue-600">
              ...
              <form className="grid md:grid-cols-2 gap-6" onSubmit={(e) => { e.preventDefault(); alert("Pendaftaran Berhasil! Silakan mulai kuis pertama Anda."); }}>
                 <div className="space-y-4">
                    <label htmlFor="reg-name" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 cursor-pointer">Nama Lengkap</label>
                    <input id="reg-name" type="text" placeholder="Nama sesuai rapor..." required className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none font-bold" />
                 </div>
                 <div className="space-y-4">
                    <label htmlFor="reg-nisn" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 cursor-pointer">NISN</label>
                    <input id="reg-nisn" type="text" placeholder="Nomor Induk Siswa..." required className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none font-bold" />
                 </div>
                 <div className="space-y-4">
                    <label htmlFor="reg-school" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 cursor-pointer">Asal Sekolah</label>
                    <input id="reg-school" type="text" placeholder="Nama sekolah lengkap..." required className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none font-bold" />
                 </div>
                 <div className="space-y-4">
                    <label htmlFor="reg-level" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 cursor-pointer">Jenjang</label>
                    <select id="reg-level" className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none font-bold">
                       <option>SD</option>
                       <option>SMP</option>
                       <option>SMA</option>
                    </select>
