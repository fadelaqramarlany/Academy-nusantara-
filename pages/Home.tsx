...
import { Trophy, BookOpen, GraduationCap, ChevronRight, Award, Zap, Brain, UserPlus, CheckCircle2, Play, RefreshCw } from 'lucide-react';
              ...
              <AnimatePresence mode="wait">
                {!isSubmitted ? (
                  ...
                  >
                    ...
                    >
                       <div className="space-y-4">
                          <label htmlFor="name" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Nama Lengkap</label>
                       ...
                       <div className="space-y-4">
                          <label htmlFor="school" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Asal Sekolah</label>
                       ...
                       <div className="space-y-4">
                          <label htmlFor="level" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Jenjang</label>
                          <select id="level" className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none font-bold transition-all">
                             <option>SD</option>
                             <option>SMP</option>
                             <option>SMA</option>
                          </select>
                       </div>
                       <button type="submit" className="md:col-span-2 bg-slate-900 text-white py-5 rounded-2xl font-black uppercase tracking-widest hover:bg-blue-600 active:scale-[0.98] transition-all shadow-xl mt-4 flex items-center justify-center gap-3">
                          Simpan Data Peserta
                       </button>
                    </form>
                  </MotionDiv>
                  ...
                  <MotionDiv
                    ...
                    aria-live="polite"
                  >
                    <div className="inline-flex items-center justify-center w-24 h-24 bg-emerald-100 text-emerald-600 rounded-full mb-8 shadow-inner">
                      <CheckCircle2 size={48} />
                    </div>
                    <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tighter mb-4">PENDAFTARAN BERHASIL!</h3>
                    <p className="text-slate-500 font-bold uppercase tracking-widest text-xs mb-10 max-w-sm mx-auto">
                      Data Anda telah tersimpan. Silakan pilih kuis di bawah untuk memulai kompetisi.
                    </p>
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                       <button
                         ...
                         className="bg-emerald-600 text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-emerald-700 active:scale-95 transition-all shadow-lg flex items-center justify-center gap-2"
                       >
                         Mulai Kuis <Play size={18} />
                       </button>
                       <button
                         onClick={() => setIsSubmitted(false)}
                         className="bg-slate-100 text-slate-600 px-8 py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-slate-200 active:scale-95 transition-all flex items-center justify-center gap-2"
                       >
                         Daftar Lagi <RefreshCw size={18} />
                       </button>
                    </div>
                  </MotionDiv>
                )}
