        ...
        <div className="max-w-4xl mx-auto">
          <form onSubmit={handleSendMessage} className="relative">
            <input 
              ...
              disabled={isLoading}
              aria-label="Ketik pesan untuk FAM AI"
            />
            <button
              type="submit"
              disabled={!input.trim() || isLoading}
              className={`absolute right-3 top-3 w-14 h-14 rounded-full flex items-center justify-center transition-all ${input.trim() && !isLoading ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-600'}`}
